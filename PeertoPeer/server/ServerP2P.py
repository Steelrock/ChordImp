...
class ServerP2P(object):
    ...
    def __init__(self, node):
        self.__HOST = node
        self.__PORT = chordLibServer.portServer
        ## Cria o Socket TCP na porta 12666
        ...
        self.stop = False
        self.node = node
        # self.client_p2p = ClientP2P(node, self.s)
        # self.__ServerNode = self
    ...
    def joinMessage(self, msg):
        ...
        if(int(typeMSG[0]) == 0):
            ...
            self.node.updateScreen("Received: ANSWER JOIN - UPDATE TO"+str(ip_node_sucessor))
    ...
    def listenToClient(self, client, address):
        size = chordLibServer.sizeObject  
        
        while True:
            #try:
                data = client.recv(size) 
                chordLibServer.totalRequisicoes = chordLibServer.totalRequisicoes + 1
                #so tratar agora o pick load para saber que é o server falando com o server e transferindo dados               
                try: 
                    ...
                    response = cPickle.dumps(localnode)
                    client.send(response)
                except:
                   if data:
                        if data.upper() == "SHOWNODES":
                            response = CH.printNodes()         
                            client.send(response)
                        if data.upper() == "SHOWSTATUS":
                            response = CH.printStatus()         
                            client.send(response)
                        if data.upper() == "SHOWFINGER":
                            response = CH.showFinger()         
                            client.send(response)                        
                        if data.upper() == "SHOWFINGERALL":
                            response = CH.showFingerAll()         
                            client.send(response)                        
                        if "JOINNODE" in data.upper():
                            CH.initJoin(data)
                            response = "Nó Adicionado com sucesso !"
                            client.send(response)
                        if data.upper() == "OLA MESTRE":
                            response = "ola\n"
                            client.send(response) 
                        else:
                        ...
                        print "Cliente " + str(address) + " disconectado !"
                        # client.close()
                        return
            #except:
                #client.close()
    ...
    def run(self):
        while(self.stop == False):
            cnn, addr = self.s.accept()
            try:
                print 'connection from', addr 
                # Receive the data in small chunks and retransmit it
                p2p = Thread(target = self.listenToClient,args = (cnn,addr))
                p2p.start()
            finally:
